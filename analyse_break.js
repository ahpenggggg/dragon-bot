...
function analyse(records) {
    ...
    for (const dim of dims) {
        ...
        for (const { len, q } of CHECKS) {
            ...
            for (const [side, s] of Object.entries(res.bySide)) {
                ...
                // Collect for summary
                // break mode — skip 续
                if (sbPct >= HIGHLIGHT) summaryHigh.push({ dim: dim.label, side, len, action:'断', pct: sbPct, n: st });
    ...
    if (summaryHigh.length === 0) {
        ...
        const largeSample = summaryHigh.filter(h => h.n > 20);
