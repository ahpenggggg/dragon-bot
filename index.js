...
// Build signal table: only 杀 (break) signals ≥ HIGHLIGHT
...
// Check if current history ends with exactly len of side
...
function buildMsg(raw, nextSignals, verResults, nextIssue) {
    ...
    // Verification of previous signals
...
async function poll() {
    ...
    if (raw.preDrawIssue!==db.lastIssue) {
        ...
        for (const ps of db.pendingSignals) {
            ...
            if (dim && dim.id.startsWith('dt')) {
                const dtF = ['firstDT','secondDT','thirdDT','fourthDT','fifthDT'];
                const idx = parseInt(dim.id.replace('dt',''))-1;
                console.log('  [debug] '+dim.id+' '+dtF[idx]+'='+raw[dtF[idx]]+' → '+actual);
            }
            console.log(`  [${hit?'HIT✅':'MISS❌'}] ${ps.label} 杀${ps.side} → ${actual}`);
            if (!hit) {
        ...
        newSigs.forEach(s=>console.log(`  [NEW⭐⭐] [杀] ${s.label} ${s.side}连${s.len}→${s.chase} ${s.pct}%`));
